...
/**
 * Phase 15 — bounded, most-recent-first list of the caller's own Student
 * Copilot conversations. A read, not a generation call: no withAiGuards, no
 * rate limit, no quota. Reuses the same non-AI-route auth convention as
 * every existing domain route (requireAuthContext/requireTenant/requireRole
 * — Phase 9/11/14's pattern), never `withAiGuards`. Label is derived from
 * the conversation's own already-persisted first query — no new AI call.
 */
export async function GET(_req: Request) {
  ...
  try {
    ...
    console.error("[ai-runtime] Failed to list Copilot conversations", err);
